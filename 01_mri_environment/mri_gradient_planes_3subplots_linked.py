#!/usr/bin/env python3
"""
MRI Gradient Coils — 3 Subplots with Independent Bz Scales + Linked Rotation
============================================================================
Each subplot shows:
  - the cylindrical bore,
  - three orthogonal planes (x=0, y=0, z=0) colored by Bz for ONE coil,
  - all three gradient coil windings (active coil opaque, others transparent).

Features requested:
  - Each axis (subplot) uses its OWN color scale (independent normalization).
  - No colorbars are drawn.
  - 3D rotations are *linked*: rotate any one subplot and the others follow.

Run:
    python mri_gradient_planes_3subplots_linked.py
...
def draw_planes(ax, Bz_x0, Bz_y0, Bz_z0, u, cmap, norm):
    ...
    surf_kwargs = dict(rstride=1, cstride=1, shade=False, antialiased=False, alpha=0.8)
    # z=0
    Xz, Yz = np.meshgrid(u, u); Zz = np.zeros_like(Xz)
...
class RotationLinker:
    """Link rotations between multiple 3D axes."""
    ...
    def on_move(self, event):
        if self._updating:
            return
        ax = event.inaxes
        if ax not in self.axes or event.button != 1:  # only while dragging with left button
            return
        # broadcast this view to others
        self._updating = True
        try:
            ...
            for other in self.axes:
                if other is ax:
                    continue
                other.view_init(elev=elev, azim=azim)
...
def main():
    ...
    # Link rotations across subplots
    linker = RotationLinker(axs)
    linker.connect(fig)
